...
| [`db.py`](db.py) | SQLite helpers: `init_db`, `get_holdings`, `save_holdings` |
...
- **Stored fields**: `company_name`, `ticker`, `shares_owned`, `avg_price`, `fees` — persisted in `portfolio.db`.
- **Calculated fields**: `current_price`, `total_cost`, `market_value`, `unrealized_pl` — derived at render time, never stored.
- **Save flow**: `app.py` reads `edited_df` from `st.data_editor`, strips empty-ticker rows, and calls `db.save_holdings()` which does a full DELETE + re-INSERT (no partial updates).
